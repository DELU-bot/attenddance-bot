...
def get_today_status():
    ...
    try:
        ...
        for row in rows:
            results.append({
                "name": row[0], "check_in": row[1], "check_out": row[2],
                "morning_status": row[3], "evening_status": row[4], "task": row[5],
                "location": row[6], "completion": row[7], "progress_status": row[8],
                "work_summary": row[9], "tasks": json.loads(row[10]) if row[10] else []
            })
        return results
    finally:
        conn.close()
...
def get_user_status(user_id):
    ...
    try:
        ...
        row = cursor.fetchone()
        if row:
            return {"name": row[0], "check_in": row[1], "check_out": row[2],
                    "status": row[3], "task": row[4], "completion": row[5], "progress_status": row[6]}
        return None
    finally:
        conn.close()
...
<style>
*{margin:0;padding:0;box-sizing:border-box}body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;background:#f5f6f7}
.header{background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);color:white;padding:20px}
.header h1{font-size:24px}.nav{background:white;padding:15px 20px;border-bottom:1px solid #e5e6e8}
.nav a{color:#3370ff;text-decoration:none;margin-right:20px;padding:8px 16px;border-radius:6px}
.nav a:hover,.nav a.active{background:#f5f7ff}.container{max-width:1200px;margin:20px auto;padding:0 20px}
.card{background:white;border-radius:12px;padding:24px;margin-bottom:20px;box-shadow:0 2px 8px rgba(0,0,0,0.08)}
.card h2{font-size:18px;margin-bottom:20px;color:#1f2329}.form-group{margin-bottom:20px}
...
.form-group textarea{min-height:100px;resize:vertical}.form-row{display:grid;grid-template-columns:repeat(3,1fr);gap:20px}
.btn{padding:10px 24px;background:#3370ff;color:white;border:none;border-radius:8px;font-size:14px;cursor:pointer}
.btn:hover{background:#2960e6}.btn-success{background:#00b365}.btn-success:hover{background:#009a55}
...
.tag .remove{margin-left:8px;cursor:pointer;color:#ff4d4f}.tag-input input{flex:1;min-width:120px}
.alert{padding:12px 16px;background:#e8f9f0;color:#00b365;border-radius:8px;margin-bottom:20px}
table{width:100%;border-collapse:collapse}th,td{padding:12px;text-align:left;border-bottom:1px solid #f0f1f3}
th{background:#f5f6f7;font-weight:500;color:#5e6e82}
</style>
</head>
<body>
<div class="header"><h1>🚗 考勤机器人管理后台</h1></div>
<div class="nav">
...
{% if page=='settings' %}
<div class="card"><h2>基本设置</h2>
<form method="post" action="/settings/save">
<div class="form-group"><label>机器人名称</label><input type="text" name="bot_name" value="{{settings.bot_name}}"></div>
<div class="form-group"><label>欢迎语</label><textarea name="welcome_message">{{settings.welcome_message}}</textarea></div>
<div class="form-group"><label>公司地址（用于定位签到）</label><input type="text" name="company_location" value="{{settings.company_location}}" placeholder="例如：北京市朝阳区建国路88号"></div>
<div class="form-row">
<div class="form-group"><label>签到有效半径（米）</label><input type="number" name="checkin_radius" value="{{settings.checkin_radius}}"></div>
<div class="form-group"><label>开启定时任务</label><select name="schedule_enabled"><option value="true" {{'selected' if settings.schedule_enabled==true else''}}>开启</option><option value="false" {{'selected' if settings.schedule_enabled==false else''}}>关闭</option></select></div>
</div><button type="submit" class="btn btn-success">保存设置</button>
</form></div>{% endif %}
{% if page=='timing' %}
<div class="card"><h2>定时任务设置</h2>
<form method="post" action="/timing/save">
<div class="form-row">
<div class="form-group"><label>早安签到提醒时间</label><input type="time" name="morning_time" value="{{settings.morning_time}}"></div>
<div class="form-group"><label>午间进度确认时间</label><input type="time" name="noon_time" value="{{settings.noon_time}}"></div>
<div class="form-group"><label>晚间签退提醒时间</label><input type="time" name="evening_time" value="{{settings.evening_time}}"></div>
</div>
<div class="form-row">
<div class="form-group"><label>日报发送时间</label><input type="time" name="report_time" value="{{settings.report_time}}"></div>
<div class="form-group"><label>周报发送时间</label><input type="time" name="week_report_time" value="{{settings.week_report_time}}"></div>
<div class="form-group"><label>月报发送时间</label><input type="time" name="month_report_time" value="{{settings.month_report_time}}"></div>
</div><button type="submit" class="btn btn-success">保存时间设置</button>
</form></div>{% endif %}
{% if page=='tasks' %}
<div class="card"><h2>任务标签管理</h2>
<form method="post" action="/tasks/save">
<div class="form-group"><label>当前任务标签</label>
<div class="tag-input" id="tagContainer">{% for tag in settings.task_tags %}<span class="tag">{{tag}}<span class="remove" onclick="removeTag(this,'{{tag}}')">×</span></span>{% endfor %}
<input type="hidden" name="task_tags" id="taskTagsInput" value="{{settings.task_tags|tojson}}">
<input type="text" id="newTag" placeholder="输入新标签后按回车添加" onkeypress="addTag(event)"></div></div>
<button type="submit" class="btn btn-success">保存任务标签</button></form></div>{% endif %}
{% if page=='status' %}
<div class="card"><h2>考勤状态管理</h2>
<form method="post" action="/status/save">
<div class="form-group"><label>考勤状态选项</label>
<div class="tag-input">{% for status in settings.status_options %}<span class="tag">{{status}}<span class="remove" onclick="removeStatus(this,'{{status}}')">×</span></span>{% endfor %}
<input type="hidden" name="status_options" id="statusInput" value="{{settings.status_options|tojson}}">
<input type="text" id="newStatus" placeholder="输入新状态后按回车添加" onkeypress="addStatus(event)"></div></div>
<button type="submit" class="btn btn-success">保存状态选项</button></form></div>{% endif %}
{% if page=='data' %}
<div class="card"><h2>考勤数据查看</h2>
<table><thead><tr><th>日期</th><th>姓名</th><th>签到时间</th><th>签退时间</th><th>状态</th><th>完成度</th></tr></thead>
<tbody>{% for record in records %}<tr><td>{{record.date}}</td><td>{{record.user_name}}</td><td>{{record.check_in_time or '-'}}</td><td>{{record.check_out_time or '-'}}</td><td>{{record.morning_status or '-'}}</td><td>{{record.completion or 0}}%</td></tr>{% endfor %}</tbody></table></div>{% endif %}
</div>
<script>
let taskTags={{settings.task_tags|tojson}};let statusOptions={{settings.status_options|tojson}};
function removeTag(el,tag){taskTags=taskTags.filter(t=>t!==tag);document.getElementById('taskTagsInput').value=JSON.stringify(taskTags);el.parentElement.remove()}
function addTag(e){if(e.key==='Enter'){e.preventDefault();const tag=document.getElementById('newTag').value.trim();if(tag&&!taskTags.includes(tag)){taskTags.push(tag);const span=document.createElement('span');span.className='tag';span.innerHTML=tag+'<span class="remove" onclick="removeTag(this,\\''+tag+'\\')">×</span>';document.getElementById('newTag').before(span);document.getElementById('taskTagsInput').value=JSON.stringify(taskTags);document.getElementById('newTag').value=''}}}
function removeStatus(el,status){statusOptions=statusOptions.filter(s=>s!==status);document.getElementById('statusInput').value=JSON.stringify(statusOptions);el.parentElement.remove()}
function addStatus(e){if(e.key==='Enter'){e.preventDefault();const status=document.getElementById('newStatus').value.trim();if(status&&!statusOptions.includes(status)){statusOptions.push(status);const span=document.createElement('span');span.className='tag';span.innerHTML=status+'<span class="remove" onclick="removeStatus(this,\\''+status+'\\')">×</span>';document.getElementById('newStatus').before(span);document.getElementById('statusInput').value=JSON.stringify(statusOptions);document.getElementById('newStatus').value=''}}}
</script></body></html>'''
...
def feishu_webhook():
    try:
        ...
        if text_content in ['签到', '/checkin', '/签到']:
            card = {"header": {"title": {"tag": "plain_text", "content": "☀️ 早安！请签到"}, "template": "blue"},
                "elements": [
            ...
            card = {"header": {"title": {"tag": "plain_text", "content": "🌙 辛苦了！请签退"}, "template": "green"},
                "elements": [
...
@app.route('/health')
def index():
    return jsonify({"status": "ok", "message": "飞书考勤机器人运行中", "time": datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
